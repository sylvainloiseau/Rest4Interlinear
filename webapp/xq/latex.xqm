...
declare
  ...
  as element(html) {
	 <html>
	 {
	  let $entry := collection($common:LexiconDataBaseName)/lift/entry[@id = $entryid]
...
declare
  ...
  as item()+ {
			...
			let $sentences := collection($common:TextsDataBaseName)/document/interlinear-text/paragraphs/paragraph/phrases/word[words/word[morphemes/morph[item[@type="cf"] = $form]][(morphemes/morph[item[@type="cf"] = $form and not(item[@type="hn"])] or morphemes/morph[item[@type="cf"] = $form and item[@type="hn"] = $order])]]
...
declare
  ...
  as item()+ {
	 ...
	 let $textnode := collection($common:TextsDataBaseName)/document/interlinear-text[item[@type = 'title-abbreviation'] = $textid]
...
\tableofcontents",
	    ...
	    for $occurrences in collection($common:TextsDataBaseName)/document/interlinear-text/paragraphs/paragraph/phrases/word/words/word/morphemes/morph[starts-with(item[@type="gls"], "DEM") or starts-with(item[@type="gls"], "this") or item[@type="msa"] = "dem"]
...
{
	 let $phrasenode := collection($common:TextsDataBaseName)/document/interlinear-text[item[@type='title-abbreviation'] = $textid]/paragraphs/paragraph/phrases/word[item[@type = 'segnum'] = $phraseid]
...
<pre>
{
    let $phrase := collection($common:TextsDataBaseName)/document/interlinear-text[item[@type='title-abbreviation'] = $textid]/paragraphs/paragraph/phrases/word[item[@type = 'segnum'] = $phraseid]
    ...
    as element(Q{http://www.w3.org/1999/xhtml}html)
{
	...
	let $tex := xslt:transform-text(collection($common:LexiconDataBaseName)/lift, "file://../xslt/lift2latex.xsl")
	let $filename := concat($common:LexiconDataBaseName, ".tex")
	let $file := file:write-text($filename, $tex)
    ...
    as element(Q{http://www.w3.org/1999/xhtml}html)
{
	...
	<html:body>{
	    ...
	    let $pdf := string-join(
		    for $entry in collection($common:LexiconDataBaseName)/lift/entry
